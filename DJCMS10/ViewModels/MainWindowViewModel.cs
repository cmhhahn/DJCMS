        ...
        public async void PlayTrack(Guid trackId)
        {
            ...
            if (track != null)
            {
                try
                {
                    ...
                    AddMixerInput(_currentTrack.Provider);
        ...
        private async Task MonitorPlayingEvents(double progress)
        {
            ...
            if ((progress > 0.9 || remainingMs < 500) && !_mixerX.MixerInputs.Any())
            {
                if (_currentTrack.Track.GapSeconds > -1)
                {
                    ...
                    if (nextTrack != null && !Buffering && _mixerX.MixerInputs.Count() < 3)
                    {
                        try
                        {
                            Buffering = true;
                            ...
                            if (_bufferTrack != null)
                            {
                                try
                                {
                                    ...
                                    Task.Run(() =>
                                    {
                                        Thread.Sleep((int)(((double)old.Track.GapSeconds + 1) * 1000.0)); // Wait for 1 second to ensure the track has started playing
                                        Buffering = false;
                                        _bufferTrack = null;
                                    });
                                    try { 
            ...
            else if (_currentTrack.Track.GapSeconds < 0 && progress > 0.5)
            {
                ...
                if (!_currentTrack.fadingOut)
                {
                    ...
                    {
                        var nextTrack = GetNextTrack();
                        if (nextTrack != null && _bufferTrack == null || _mixerX.MixerInputs.Count() < 3)
                        {
                            try
                            {
